...
if (!$class_subject_info) { /* ... redirect ถ้าไม่เจอ ... */ }

$subject_credits = $class_subject_info['credits'];
$is_pass_fail_subject = ((float)$subject_credits == 0);


// 3. **แก้ไข:** จัดการการบันทึกข้อมูล (POST Request) ให้สมบูรณ์
    ...
    try {
        if ($is_pass_fail_subject) {
            ...
            foreach ($scores as $student_id => $score) {
                ...
                $grade_info = calculate_grade_from_score($score_value, $subject_credits);
...
$sql_students = "SELECT s.student_id, s.student_code, s.prefix, s.first_name, s.last_name, e.score, e.grade
                 ...
                 WHERE s.class_id = ? ORDER BY s.student_code ASC";
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <h5 class="text-muted">วิชา: <?php echo htmlspecialchars($class_subject_info['subject_name']); ?> | ห้องเรียน: <?php echo htmlspecialchars($class_subject_info['class_name']); ?></h5>
                </div>
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    <?php if(empty($students)): ?>
                                        <tr><td colspan="3" class="text-center py-4">ไม่พบนักเรียนในคลาสนี้</td></tr>
                                    <?php else: ?>
